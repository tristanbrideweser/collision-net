"""
demo_neural_rrt.py — Demo neural RRT-Connect for presentation
"""
...
def main():
    ...
    parser.add_argument("--scene-type", default="tabletop")
    ...
    p.resetDebugVisualizerCamera(1.8, 45, -25, [0.3, 0, 0.4])
    ...
    if goal is None:
        print("No collision-free goal found!")
    ...
    print("\nPlanning...")
    t0 = time.time()
    path = rrt_connect(start, goal, neural_checker)
    ...
    if n_bad == 0:
        ...
        print(f"  ⚠️ {n_bad}/{len(path)} in collision (geometric)")
    
    input("\n>>> Press Enter to execute...")
    
    # Execute
    smooth = interpolate_path(path)
